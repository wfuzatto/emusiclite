...
def render_neural(req: NeuralReq):
    ...
    try:
        result = neuralize_reference(
            reference["final"], genre=genre, prompt=req.prompt, bpm=req.bpm,
            seconds=req.seconds, candidates=req.candidates,
            cover_strength=req.cover_strength, exploration=req.exploration,
            instrumental=req.instrumental, lyrics=req.lyrics,
        )
        target = OUTPUT / f"{reference['job']}-{genre}-neural04.wav"
        ...
        manifest = {
            ...
            "prompt": req.prompt,
            "bpm": req.bpm,
            "reference_audio": reference["final"],
            "final": str(target),
            "neural_backend": result.get("backend"),
            "best": result.get("best"),
            "candidates": result.get("candidates", []),
            "candidate_failures": result.get("failures", []),
            "reference": reference,
        }
        (work / "neural-manifest.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False), encoding="utf-8")
        return manifest
    except Exception as exc:
