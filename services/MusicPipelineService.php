...
class MusicPipelineService
{
    ...
    public function process(array $job): array
    {
        ...
        if (!$track['plan_saved_at']) {
            ...
            $plan = $this->withHeartbeat($job, 'Criando planejamento', fn() => (new ResourceGuard($this->db, $this->config))->withTextSlot(fn() => (new TextGenerationService($this->db, $this->config))->plan($track)));
            $this->tracks->savePlan($trackId, $plan); $track = $this->loadTrack($trackId);
        }
        ...
        if (!$track['lyrics_saved_at']) {
            ...
            if (!$instrumental && $lyrics === '') {
                ...
                $lyrics = $this->withHeartbeat($job, 'Criando letra', fn() => (new ResourceGuard($this->db, $this->config))->withTextSlot(fn() => (new TextGenerationService($this->db, $this->config))->generateLyrics($track, $plan)));
            }
            $this->stage($job, 'Salvando letra', 28);
            $this->tracks->saveLyrics($trackId, (string) $plan['lyrical_concept'], $lyrics, $track['language'], $instrumental); $track = $this->loadTrack($trackId);
        }

        if (!$track['prompt_saved_at']) { $this->stage($job, 'Persistindo prompt musical', 33); $this->tracks->markPromptSaved($trackId); $track = $this->loadTrack($trackId); }

        $state = json_decode((string) ($track['generation_state'] ?? ''), true) ?: [];
        $generationModel = (string) ($state['generation_model'] ?? 'turbo');
        ...
        if (!$track['audio_validated_at']) { $this->stage($job,'Validando áudio',65); $validation=(new AudioValidationService($this->config))->validate($audioPath,(int)$track['desired_duration_seconds']); $state=array_merge($state,['audio_path'=>$audioPath,'audio_validation'=>$validation]); $this->tracks->markCheckpoint($trackId,'audio_validated_at','Áudio validado',70,$state); $track=$this->loadTrack($trackId); }
        if (!$track['audio_uploaded_at'] || !$track['audio_media_id']) { $this->stage($job,'Enviando áudio',73); $mediaId=(new IngestClient($this->config))->upload('AUDIO',$trackId,(int)$track['owner_user_id'],$audioPath,$state['audio_validation']??[]); $stmt=$this->db->prepare('UPDATE music_ai_tracks SET audio_media_id=? WHERE id=?');$stmt->bind_param('ii',$mediaId,$trackId);$stmt->execute();$stmt->close();$this->tracks->markCheckpoint($trackId,'audio_uploaded_at','Áudio armazenado',78,$state);$track=$this->loadTrack($trackId); }
        ...
        $stmt=$this->db->prepare("UPDATE music_ai_tracks SET status='COMPLETED',current_stage='Pronta para publicar',progress_percent=95,last_error=NULL WHERE id=? AND audio_media_id IS NOT NULL AND cover_media_id IS NOT NULL");$stmt->bind_param('i',$trackId);$stmt->execute();$stmt->close();
        $payload=json_decode((string)($job['payload']??''),true)?:[]; if(!empty($payload['auto_publish'])&&$track['source']==='AGENT')$this->tracks->publish($trackId,(int)$track['owner_user_id']);
        $this->updateAgent((int)($job['agent_run_id']??0),$trackId); return ['track_id'=>$trackId,'completed'=>true];
    }

    private function stage(array $job,string $stage,int $percent):void{$trackId=(int)$job['track_id'];$jobId=(int)$job['id'];$stmt=$this->db->prepare("UPDATE music_ai_tracks SET status='GENERATING',current_stage=?,progress_percent=? WHERE id=?");$stmt->bind_param('sii',$stage,$percent,$trackId);$stmt->execute();$stmt->close();$this->queue->touchJob($jobId,$this->workerId);$this->queue->heartbeat($this->workerId,'PROCESSING',$jobId,$trackId,$stage,null);if(!empty($job['agent_run_id'])){$run=(int)$job['agent_run_id'];$stmt=$this->db->prepare("UPDATE music_ai_agent_runs SET status='RUNNING',current_stage=?,progress_percent=?,attempts=GREATEST(attempts,1),last_heartbeat_at=NOW() WHERE id=?");$stmt->bind_param('sii',$stage,$percent,$run);$stmt->execute();$stmt->close();}}
    ...
    private function withHeartbeat(array $job,string $stage,callable $operation){$process=null;if(function_exists('proc_open')){$null=fopen('/dev/null','a');$process=@proc_open([PHP_BINARY,__DIR__.'/../workers/heartbeat_lease.php',$this->workerId,(string)(int)$job['id'],(string)(int)$job['track_id'],$stage,(string)getmypid()],[0=>['file','/dev/null','r'],1=>$null,2=>$null],$pipes);if(is_resource($null))fclose($null);}try{return $operation();}finally{if(is_resource($process)){@proc_terminate($process);@proc_close($process);}}}
}
