...
$input=[];foreach(['genre','subgenre','mood','theme','language','composition_type','voice_type','instruments','descriptive_references','original_idea','desired_duration_seconds','bpm'] as $key)$input[$key]=is_string($_POST[$key]??null)?trim($_POST[$key]):($_POST[$key]??null);
$allowedTypes=['INSTRUMENTAL','VOCAL'];$allowedLanguages=['Português','Inglês','Espanhol','Francês','Italiano','Alemão','Sem idioma'];
...
$duration=(int)$input['desired_duration_seconds'];if($duration<30||$duration>600)music_json(['ok'=>false,'message'=>'Escolha duração entre 30 segundos e 10 minutos.'],422);
