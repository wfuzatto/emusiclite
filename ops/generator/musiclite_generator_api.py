...
XL_INFERENCE_STEPS = int(os.environ.get("MUSIC_AI_GENERATOR_XL_STEPS", "50"))
...
def _select_quantization() -> str | None:
    if QUANTIZATION in {"none", "off", "false", "0"}: return None
    if QUANTIZATION != "auto":
        if QUANTIZATION not in {"int8_weight_only", "w8a8_dynamic"}: raise RuntimeError("MUSIC_AI_GENERATOR_QUANTIZATION possui valor inválido.")
        return QUANTIZATION
    try:
        import torch
        if torch.cuda.is_available():
            major, _ = torch.cuda.get_device_capability(0)
            if major < 7: return "w8a8_dynamic"
    except Exception:
...
def _authorize(authorization: str | None) -> None:
    ...
    if not authorization or not hmac.compare_digest(authorization, expected): raise HTTPException(status_code=401, detail="Não autorizado.")
...
def _dispose_handler(handler) -> None:
    if handler is not None: del handler
    gc.collect()
    try:
        import torch
        if torch.cuda.is_available():
            torch.cuda.empty_cache(); torch.cuda.ipc_collect()
    except Exception: LOGGER.warning("Could not clear MusicLite CUDA cache", exc_info=True)


def _get_handler(model_name: str):
    global _dit_handler, _dit_model_name
    if _dit_handler is not None and _dit_model_name == model_name: return _dit_handler
    with _handler_lock:
        if _dit_handler is not None and _dit_model_name == model_name: return _dit_handler
        if not _model_installed(model_name): raise HTTPException(status_code=422, detail=f"O modelo {model_name} ainda não está instalado no servidor.")
        old_handler = _dit_handler; _dit_handler = None; _dit_model_name = None; _dispose_handler(old_handler)
        from acestep.handler import AceStepHandler
        handler = AceStepHandler(); quantization = _select_quantization(); experimental_xl = model_name == GENERATION_MODELS["xl_sft"]
        LOGGER.info("Initializing ACE-Step model=%s quantization=%s experimental_xl=%s", model_name, quantization or "none", experimental_xl)
        status, ready = handler.initialize_service(
            project_root="/opt/musiclite/generator",
            config_path=model_name,
            device="cuda",
            use_flash_attention=False,
            compile_model=not experimental_xl,
            offload_to_cpu=True,
        ...
        if not ready:
            LOGGER.error("ACE-Step initialization failed for %s: %s", model_name, status); _dispose_handler(handler)
            raise RuntimeError("O gerador não pôde inicializar o modelo selecionado.")
        _dit_handler = handler; _dit_model_name = model_name; return handler


def _release_own_handler() -> None:
    global _dit_handler, _dit_model_name
    with _handler_lock:
        handler = _dit_handler; _dit_handler = None; _dit_model_name = None
    _dispose_handler(handler)


@app.get("/health")
def health(authorization: str | None = Header(default=None)) -> dict[str, Any]:
    _authorize(authorization); free_vram = _free_vram_mb()
...
def generate(payload: dict[str, Any], authorization: str | None = Header(default=None)) -> dict[str, str]:
    _authorize(authorization)
    try: spec = build_ace_spec(payload, max_duration=MAX_DURATION)
    except ValueError as exc: raise HTTPException(status_code=422, detail=str(exc)) from exc
    if not spec["instrumental"] and _select_quantization() == "w8a8_dynamic": raise HTTPException(status_code=422, detail="Configuração de GPU insuficiente para geração vocal.")
    if not _generation_lock.acquire(blocking=False): raise HTTPException(status_code=409, detail="Já existe uma geração em andamento.")
    try:
        free_vram = _free_vram_mb()
        if free_vram is None or free_vram < MIN_FREE_VRAM_MB: raise HTTPException(status_code=503, detail="Recursos temporariamente indisponíveis.")
        from acestep.inference import GenerationConfig, GenerationParams, generate_music
        from acestep.llm_inference import LLMHandler
        job_id = str(uuid.uuid4()); job_dir = OUTPUT_ROOT / job_id; job_dir.mkdir(mode=0o750)
        steps = XL_INFERENCE_STEPS if spec["model_key"] == "xl_sft" else INFERENCE_STEPS
        params = GenerationParams(task_type="text2music", caption=spec["caption"], lyrics=spec["lyrics"], instrumental=spec["instrumental"], vocal_language=spec["vocal_language"], bpm=spec["bpm"], keyscale=spec["keyscale"], timesignature=spec["timesignature"], duration=spec["duration"], inference_steps=steps, thinking=False, use_cot_metas=False, use_cot_caption=False, use_cot_lyrics=False, use_cot_language=False, dcw_enabled=_dcw_enabled(), velocity_norm_threshold=_velocity_norm_threshold())
        ...
        if not result.success or not result.audios: raise HTTPException(status_code=503, detail="A geração de áudio falhou temporariamente.")
        generated = Path(str(result.audios[0].get("path", ""))).resolve()
        if not generated.is_file() or job_dir not in generated.parents or generated.stat().st_size < 1024: raise HTTPException(status_code=503, detail="O gerador não produziu um arquivo válido.")
        ...
        LOGGER.exception("Unexpected generation failure"); _release_own_handler(); raise HTTPException(status_code=503, detail="A geração de áudio falhou temporariamente.")
...
if __name__ == "__main__": uvicorn.run(app, host="127.0.0.1", port=int(os.environ.get("MUSIC_AI_GENERATOR_PORT", "8091")), workers=1)
