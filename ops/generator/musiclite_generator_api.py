...
def _release_own_handler() -> None:
    """Release only this service's ACE-Step model after a failed generation."""
    global _dit_handler
    with _handler_lock:
...
def generate(payload: dict[str, Any], authorization: str | None = Header(default=None)) -> dict[str, str]:
    ...
    # ACE-Step vocal conditioning needs float32 on this pre-Volta profile.
    # The isolated 6 GB GPU cannot hold that model even with ComfyUI stopped,
    # so do not consume durable-queue retries for a known configuration limit.
